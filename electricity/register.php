...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!preg_match("/^[a-zA-Z0-9]+$/", $registration_number)) {
        ...
        $sql = "INSERT INTO users (registration_number, name, dob, connection_date, address, role, password, connection_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssss", $registration_number, $name, $dob, $connection_date, $address, $role, $password, $connection_type);
